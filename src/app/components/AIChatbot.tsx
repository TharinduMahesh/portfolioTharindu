...
export default function AIChatbot() {
  ...
  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50">
      {/* Chat Window */}
      {isOpen && (
        <div className="w-[calc(100vw-2rem)] sm:w-96 h-[calc(100vh-8rem)] sm:h-[550px] max-h-[600px] shadow-2xl rounded-2xl flex flex-col overflow-hidden bg-gradient-to-br from-[#0a0a0a] to-[#1a1a1a] border border-white/10 backdrop-blur-xl">
          {/* Header */}
          <div className="relative flex justify-between items-center p-4 sm:p-5 border-b border-white/10 bg-gradient-to-r from-purple-500/10 to-pink-500/10">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                <span className="text-white font-bold text-base sm:text-lg">AI Assistant</span>
          ...
          {/* Messages */}
          <div className="flex-grow overflow-y-auto p-3 sm:p-5 space-y-3 sm:space-y-4 scrollbar-thin scrollbar-thumb-purple-500/20 scrollbar-track-transparent">
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center px-4 sm:px-6">
                <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-gradient-to-br from-purple-500/20 to-pink-500/20 flex items-center justify-center mb-3 sm:mb-4">
            ...
            {messages.map((m) => (
              <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"} animate-fadeIn`}>
                <div
                  className={`max-w-[85%] p-3 sm:p-4 rounded-2xl text-xs sm:text-sm leading-relaxed ${
                    m.role === "user"
          ...
          {/* Input */}
          <form onSubmit={handleSubmit} className="p-3 sm:p-4 border-t border-white/10 bg-gradient-to-r from-purple-500/5 to-pink-500/5">
            <div className="flex gap-2">
              <input
                className="flex-grow p-3 sm:p-4 rounded-xl bg-white/5 border border-white/10 focus:border-purple-500/50 focus:ring-2 focus:ring-purple-500/20 outline-none text-xs sm:text-sm text-white placeholder-gray-500 transition-all backdrop-blur-sm"
                value={input}
              ...
              <button
                ...
                className="p-3 sm:p-4 rounded-xl bg-gradient-to-br from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-purple-500/20 hover:shadow-purple-500/40 hover:scale-105"
              >
      ...
      <button
        onClick={toggleChat}
        className="group w-14 h-14 sm:w-16 sm:h-16 rounded-full shadow-2xl bg-gradient-to-br from-purple-600 via-pink-500 to-purple-600 flex items-center justify-center hover:scale-110 transition-all duration-300 hover:shadow-purple-500/50 relative overflow-hidden"
      >
        ...
        <ChatBubbleLeftIcon className="h-7 w-7 sm:h-8 sm:w-8 text-white relative z-10" />
        {!isOpen && (
